...
In this example however, when `5` is appended to `g` it overwrites `j`'s fourth index because `j` and `g` point to the _same underlying array_. The `append()` function only creates a new array when there isn't any capacity left. We created `i` with a length of 3 and a capactiy of 8, which means we can append `5` items before a new array is automatically allocated.
